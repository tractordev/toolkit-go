...
func (w *Window) Load() {
	...
	if len(w.Options.Icon) > 0 {
		icon = CreateIconFromBytes(w.Options.Icon)
	}
	...
	x := int32(w.Options.Position.X)
	y := int32(w.Options.Position.Y)
	ww := int32(0)
	h := int32(0)

	hwnd := CreateWindowExW(0, apptronClassName, w.Options.Title, style, x, y, ww, h, 0, 0, GetModuleHandle(), 0)
	if hwnd == 0 {
		panic(ErrCreateWindow)
	}
	...
	size := mulSize(w.Options.Size, scale)
	...
	//chromium.DataPath = w.Options.DataPath
	...
	if w.Options.URL != "" {
		chromium.Navigate(w.Options.URL)
	}

	if w.Options.HTML != "" {
		chromium.Navigate("data:text/html, " + w.Options.HTML)
	}

	if w.Options.Script != "" {
		chromium.Eval(w.Options.Script)
	}
	...
	w.hasMenu = hasMenu
	w.scale = scale
	w.minSize = POINT{X: LONG(w.Options.MinSize.Width), Y: LONG(w.Options.MinSize.Height)}
	w.maxSize = POINT{X: LONG(w.Options.MaxSize.Width), Y: LONG(w.Options.MaxSize.Height)}
	w.isTransparent = w.Options.Transparent
	w.ID = w.Options.ID

	chromium.MessageCallback = w.messageCallback
	//chromium.Eval("window.chrome.webview.postMessage('Hello, sir!');")

	SetWindowLongPtrW(hwnd, GWLP_USERDATA, unsafe.Pointer(w))
