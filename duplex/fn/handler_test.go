...
func TestHandlerFromFunc(t *testing.T) {
	...
	t.Run("channel return value stream", func(t *testing.T) {
		client, _ := rpctest.NewPair(HandlerFrom(func() (chan string, string) {
			ch := make(chan string)
			go func() {
				ch <- "one"
				ch <- "two"
		...
		var ret string
		resp, err := client.Call(ctx, "", nil, &ret)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if ret != "ok" {
		...
		go rpc.ReceiveNotify(ctx, resp, ch)
		var vals []string
		for s := range ch {
			vals = append(vals, s)
		}
		if !reflect.DeepEqual(vals, []string{"one", "two"}) {
			t.Fatalf("unexpected streamed values: %v", vals)
		}

	})

	t.Run("channel arg stream", func(t *testing.T) {
		client, _ := rpctest.NewPair(HandlerFrom(func(ch chan string) string {
			go func() {
				ch <- "one"
				ch <- "two"
				close(ch)
