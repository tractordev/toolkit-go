...
func RegexFilterHook(r *regexp.Regexp, useFullPath bool) FilterFileHookFunc {
	return func(info fs.FileInfo, fullPath string) error {
		str := info.Name()
...
type Watcher struct {
	...
	files        map[string]fs.FileInfo // map of files.
	...
	fs           fs.StatFS              // filesystem to use
}

// New creates a new Watcher.
func New(fsys fs.FS) *Watcher {
	sfs := fsys.(fs.StatFS) // for now, let panic if not StatFS
...
func (w *Watcher) list(name string) (map[string]fs.FileInfo, error) {
	fileList := make(map[string]fs.FileInfo)
	...
	// in case the fs is passing it by
	// reference and the values will just
	// update. we need values at this time.
...
func (w *Watcher) listRecursive(name string) (map[string]fs.FileInfo, error) {
	fileList := make(map[string]fs.FileInfo)

	return fileList, fs.WalkDir(w.fs, name, func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
...
func (w *Watcher) retrieveFileList() map[string]fs.FileInfo {
	w.mu.Lock()
	defer w.mu.Unlock()
...
func (w *Watcher) pollEvents(files map[string]fs.FileInfo, evt chan Event,
	cancel chan struct{}) {
	w.mu.Lock()
	defer w.mu.Unlock()

	// Store create and remove events for use to check for rename events.
	creates := make(map[string]fs.FileInfo)
	removes := make(map[string]fs.FileInfo)
